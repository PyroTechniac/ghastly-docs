  ...
  >
          ...
          <div class="level-item has-text-left">
            ...
            <span v-if="method.async">
              <span
                ...
                title="This method is async, and returns a promise."
              >Async</span>&nbsp;
            </span>
            <span v-if="method.abstract">
              <span
                class="tag is-info"
                title="This method is abstract, and should be implemented in child classes."
              >Abstract</span>&nbsp;
            </span>
            ...
            <span v-if="method.generator">
              <span
                class="tag is-dark"
                title="This method returns a generator function."
              >Generator</span>&nbsp;
            </span>
          </div>
          <span class="level-item has-text-left is-marginless">
            <router-link
              :to="{ name: 'docs-class', query: { scrollTo } }"
            >.{{ method.name }}({{ params }})</router-link>
          </span>
        </div>
      </nav>
      <source-button
    ...
    <div class="card-content">
      ...
      <div v-if="method.examples">
        ...
        <pre
          v-for="(example, key) in method.examples"
          :key="key"
          v-highlightjs
        ><code class="javascript">{{ example }}</code></pre>
      </div>
    </div>
    <footer class="card-footer">
      ...
      >
        <span>
          ...
          <span v-if="method.yields">
            <span
              v-for="(rtrn, index) of method.yields.types || method.yields"
              :key="index"
            >
              <types
                :names="rtrn"
                :variable="method.yields.variable"
                :nullable="method.yields.nullable"
                :docs="docs"
              />
      ...
      <p
        v-if="!method.yields || method.returns"
        class="card-footer-item"
      >
        <span>
          ...
          <span v-if="method.returns">
            <span
              v-for="(rtrn, index) of method.returns.types || method.returns"
              :key="index"
            >
              <types
                :names="rtrn"
      ...
      >
        <span>
          ...
          <span
            v-for="(thrw, index) of method.throws"
            :key="index"
          >
            <types
              :names="thrw"
...
export default {
  ...
  computed: {
    params() {
      ...
      return paramListing(
        this.method.params.filter(par => !par.name.includes("."))
      );
    },
    description() {
      return Vue.filter("marked")(
        convertLinks(
    ...
    emits() {
      ...
      return this.method.emits.map(emit =>
        parseLink(emit.replace(/:event/i, ""), undefined, this.docs)
      );
    },
    scrollTo() {
