  ...
  >
    <template slot-scope="props">
      ...
      <b-table-column label="Type">
        <span
          v-for="(type, index) of props.row.type"
          :key="index"
        >
          <types
            :names="type"
            :variable="props.row.variable"
            :nullable="props.row.nullable"
            :docs="docs"
          />
  ...
  methods: {
    paramDescription(param) {
      return Vue.filter("marked")(
        convertLinks(param.description, this.docs, this.$router, this.$route)
      );
    },
    paramDefault(param) {
