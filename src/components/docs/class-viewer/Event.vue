<template>
  ...
  >
      ...
      <nav class="card-header-title level is-marginless">
        <div class="level-left">
          ...
          <span class="level-item has-text-left is-marginless">
            <router-link
              :to="{ name: 'docs-class', query: { scrollTo: event.name } }"
    ...
    <div class="card-content">
      ...
      />
      <param-table
        v-if="event.params && event.params.length"
        :params="params"
        :docs="docs"
      />
    </div>
...
export default {
  ...
  computed: {
    ...
    params() {
      ...
      for (const param of this.event.params) {
        params.push(param);
        if (param.type[0][0][0]) {
          params.push(...this.findTypeDefs(param.type[0][0][0], param.name));
        }
      }
      return params;
    }
  ...
  methods: {
    findTypeDefs(name, prefix) {
      ...
      if (typedef.type[0][0][0]) {
        props.push(...this.findTypeDefs(typedef.type[0][0][0], prefix));
      }
      return props;
    }
  }
