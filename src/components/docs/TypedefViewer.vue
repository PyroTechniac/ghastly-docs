<template>
  <div v-if="typedef">
    <div class="card">
      <header class="card-header">
        <nav class="card-header-title level is-marginless">
          <div class="level-left">
            ...
            <span class="level-item is-size-3 is-marginless">{{ typedef.name }}&nbsp;</span>
          </div>
        </nav>
        <source-button
          :meta="typedef.meta"
      ...
      <div
        v-if="typedef.description || (typedef.props && typedef.props.length)"
        class="card-content"
      >
        <p
      ...
      <footer class="card-footer">
        <p class="card-footer-item">
          <span>
            <strong>Type:</strong>
            <span
              v-for="(type, index) of typedef.type"
              :key="index"
            >
              <types
                :names="type"
...
export default {
  ...
  data() {
    return {
      typedef: this.docs.typedefs.find(
        typ => typ.name === this.$route.params.typedef
      )
    };
  },
  computed: {
