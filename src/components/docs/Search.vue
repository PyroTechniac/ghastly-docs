  ...
  <div>
    ...
    >
      <b-checkbox-button
        v-model="toggles"
        :type="tagColor('Class')"
        native-value="classes"
      >
        Classes
      </b-checkbox-button>
      <b-checkbox-button
        v-model="toggles"
        :type="tagColor('Property')"
        native-value="props"
      >
        Properties
      </b-checkbox-button>
      ...
      <b-checkbox-button
        v-model="toggles"
        :type="tagColor('Typedef')"
        native-value="typedefs"
      >
        Typedefs
      </b-checkbox-button>
    </b-field>
      ...
      <div v-if="search && search.length > 2">
        <div v-if="exactMatches.length">
          ...
          >
            ...
            >
              <SearchResult
                v-for="result in exactMatches"
                :key="`${result.key || result.name}`"
                :search="search"
                :result="result"
                :scores="showScores"
                :color="tagColor(result.badge)"
              />
            </transition-group>
          </transition>
          <br>
        ...
        <div v-if="partialMatches.length">
          ...
          >
            ...
            >
              <SearchResult
                v-for="result in partialMatches"
                :key="`${result.key || result.name}`"
                :search="search"
                :result="result"
                :scores="showScores"
        ...
        <h2
          v-if="!exactMatches.length && !partialMatches.length"
    ...
    results() {
      ...
      for (const clarse of this.docs.classes) {
        ...
        if (this.toggles.includes("classes")) {
          cScore =
            searchScore(query, clarse.name.toLowerCase(), null, 1) * 1.05;
        ...
        for (const [group, groupName] of [
          ...
          for (const item of clarse[group]) {
            ...
            const score = searchScore(
              ...
              cScore <= 0.9 ? name.toLowerCase() : null
            );
            ...
            results.push({
              ...
              route: {
                ...
                query: { scrollTo: this.scopedName(item) }
              },
              badge: groupName,
      ...
      if (this.toggles.includes("typedefs")) {
        for (const typedef of this.docs.typedefs) {
          ...
          const tScore =
            searchScore(query, typedef.name.toLowerCase(), null, 1) * 1.05;
  ...
  watch: {
    ...
    search(q) {
      ...
      if (this.$route.query.q) {
        this.$router.replace({ name: "docs-search", query: { q } });
      } else {
        this.$router.push({ name: "docs-search", query: { q } });
      }
    }
    /* eslint-enable id-length max-statements-per-line */
  },
  methods: {
    tagColor(type) {
...
function searchScore(query, shortName, longName, identicalWeight) {
  if (query === shortName || query === longName) {
    return 1 + (identicalWeight === undefined ? 0.5 : identicalWeight);
  }
