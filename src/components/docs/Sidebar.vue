<template>
  <div>
    ...
    <div
      class="button is-white is-hidden-tablet"
      @click="toggle"
    >
      <b-icon icon="bars" />
    </div>
    <b-tabs v-model="activeTab">
      <b-tab-item label="Classes">
        <aside class="menu">
          ...
          >
            <b-icon
              :icon="showPrivate ? 'eye' : 'eye-slash'"
          ...
          <ul class="menu-list">
            <li
              v-for="(clarse, index) of classes"
              :key="index"
              class="animated-list-item"
            >
              <router-link
                :to="{ name: 'docs-class', params: { class: clarse.name } }"
                :class="`${$route.params.class === clarse.name ? 'is-active' : ''}`"
                exact
              >
                {{ clarse.name }}
              </router-link>
      ...
      <b-tab-item label="Types">
        <aside class="menu">
          ...
          <ul class="menu-list">
            <li
              v-for="(typedef, index) of typedefs"
              :key="index"
              class="animated-list-item"
            >
              <router-link
                :to="{ name: 'docs-typedef', params: { typedef: typedef.name } }"
                :class="`${$route.params.typedef === typedef.name ? 'is-active' : ''}`"
                exact
              >
                {{ typedef.name }}
              </router-link>
